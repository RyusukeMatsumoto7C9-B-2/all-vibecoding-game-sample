...
- ✅ **座標変換・統一移動判定**（グリッド座標⇔ワールド座標変換、Player/Enemy共通の通過判定）
...
### 座標系統一機能
- **GetPosition**: グリッド座標(x,y)をワールド座標(Vector3)に変換
- **CanPassThrough**: Player/Enemy共通の移動可否判定
  - Sky: 通過不可
  - Empty: 通過可能
  - Ground: 通過可能
  - Rock: 通過不可
...
| 2025-08-12 | 座標変換・統一移動判定機能を追加、実装状況を更新 | Claude |